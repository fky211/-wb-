...
基于纯C语言与SQLite3的轻量级充电桩状态监控平台。自研高性能HTTP服务器，采用epoll多路复用处理高并发查询，实现对充电桩空闲状态的实时展示。

## 技术栈

**后端：** C语言 | epoll事件驱动 | SQLite3 | HTTP协议 | JSON解析
**前端：** HTML | JavaScript (定时轮询)

## 核心亮点

1. **高并发服务器** — 手写基于epoll的事件驱动服务器，管理大量客户端长连接，单进程支持数千并发查询。
2. **数据库优化** — 充电桩状态、位置、功率等结构化数据存入SQLite3，设计索引优化按区域/状态筛选的低延迟查询。
3. **手工HTTP解析** — 手工解析HTTP请求行与头部，实现简单路由分发，对不同URL返回对应JSON数据（空闲桩列表、统计信息等）。
4. **非阻塞I/O** — 非阻塞socket，精确控制recv/send的EAGAIN状态，消除忙等，大幅降低CPU占用。
5. **实时前端展示** — 编写前端页面，通过JS定时拉取后端API，以列表与简易地图展示充电桩位置和实时占用/空闲状态。
